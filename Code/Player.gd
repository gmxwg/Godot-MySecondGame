...
var GroundState = [State.RUNNING,State.IDLE,State.LANDING,State.ATTACK1,State.ATTACK2,State.ATTACK3]
...
func _unhandled_input(event: InputEvent) -> void: # 在落地前0.1内秒按下按键也可以实现跳跃，不用等落地再按
	...
	if Input.is_action_just_pressed("attack") and CanCombo:
		ComboRequested = true
		
func tick_physics(state : State, delta: float) -> void: # 每一帧要做的事情
	match state:
		State.IDLE:
...
func get_next_state(state : State) -> State: # 判断状态是否转移
	...
	if ShouldJump:
		return State.JUMP
	if state in GroundState and not is_on_floor(): # 优先处理，因为Combo也是在地面，Fall应该先判断
		return State.FALL
	...
	match state:
		...
		State.WALL_JUMP:
			...
			if velocity.y > 0:
				return State.FALL
		State.ATTACK1:
			if not $AnimationPlayer.is_playing(): # 其他状态切换不需要播放完动画
				if ComboRequested:
...
func transition_state(from : State, to : State) -> void: # 每次状态转移要做的事情，每次状态转移时调用，每一帧都会检查状态切换，根据状态播放动画
